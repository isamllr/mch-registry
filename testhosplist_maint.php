<?php
//Include Common Files @1-26D94508
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "testhosplist_maint.php");
...
//Class_Initialize Event @3-22D15102
    function clsRecordpregnancy_header($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->hospitalisation = new clsControl(ccsLabel, "hospitalisation", "hospitalisation", ccsText, "", CCGetRequestParam("hospitalisation", $Method, NULL), $this);
            $this->AdmissionDate = new clsControl(ccsLabel, "AdmissionDate", "AdmissionDate", ccsDate, array("ShortDate"), CCGetRequestParam("AdmissionDate", $Method, NULL), $this);
        }
    }
...
//CheckErrors Method @3-E8A14BDB
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->hospitalisation->Errors->Count());
...
//Show Method @3-7FDC81CB
    function Show()
    {
        ...
        $this->hospitalisation->SetText($CCSLocales->GetText("HHospitalisation"));
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->hospitalisation->Errors->ToString());
...
//DataSource Variables @3-C05312E2
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @3-8E7B6857
    function clspregnancy_headerDataSource(& $Parent)
    {
        ...
        $this->hospitalisation = new clsField("hospitalisation", ccsText, "");
...
//Prepare Method @3-2576548A
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlHospitalisationID", ccsInteger, "", "", $this->Parameters["urlHospitalisationID"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "hospitalisation.HospitalisationID", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsInteger),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @3-64F87FE0
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM (pregnancy INNER JOIN patient ON\n\n" .
        "pregnancy.PatientID = patient.PatientID) INNER JOIN hospitalisation ON\n\n" .
        "hospitalisation.PregnancyID = pregnancy.PregnancyID {SQL_Where} {SQL_OrderBy}";
...
//Class_Initialize Event @495-8A67E4FB
...
//Initialize Method @495-BE1B0C93
    function Initialize()
    {
...
//CheckErrors Method @495-629F02AA
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->PregnancyRecNr->Errors->Count());
...
//DataSource Variables @495-AC5E0F34
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @495-D80A4172
...
//Class_Initialize Event @343-30836533
    function clsEditableGridtests($RelativePath, & $Parent)
    {
        ...
        $this->CachedColumns["TestHospitalisationID"][0] = "TestHospitalisationID";
...
//Initialize Method @343-73AE80CA
    function Initialize()
    {
        if(!$this->Visible) return;
...
//Validate Method @343-287332C2
    function Validate()
    {
        ...
        for($this->RowNumber = 1; $this->RowNumber <= $this->TotalRows; $this->RowNumber++)
        {
            ...
            $this->DataSource->CachedColumns["TestHospitalisationID"] = $this->CachedColumns["TestHospitalisationID"][$this->RowNumber];
...
//Operation Method @343-E9E20B2E
    function Operation()
    {
        ...
        $Redirect = "hospitalisation_maint.php" . "?" . CCGetQueryString("QueryString", array("ccsForm"));
...
//UpdateGrid Method @343-E1B63241
    function UpdateGrid()
    {
...
//SetFormState Method @343-A1A1ACA8
    function SetFormState($FormState)
    {
        if(strlen($FormState)) {
            ...
            for($i = 2; $i < sizeof($pieces); $i = $i + 3)  {
                ...
                $piece = str_replace("\\" . ord("\\"), "\\", $piece);
                $piece = str_replace("\\" . ord(";"), ";", $piece);
                $this->CachedColumns["TestHospitalisationID"][$RowNumber] = $piece;
                $RowNumber++;
            }
            ...
            for($i = 1; $i <= $this->EmptyRows; $i++) {
                ...
                $this->CachedColumns["TestHospitalisationID"][$RowNumber] = "";
                $RowNumber++;
            }
        }
    }
...
//GetFormState Method @343-95E70987
        ...
        if(!$this->FormSubmitted) {
            ...
            if($NonEmptyRows) {
                for($i = 0; $i <= $NonEmptyRows; $i++) {
                    ...
                    $this->FormState .= ";" . str_replace(";", "\\;", str_replace("\\", "\\\\", $this->CachedColumns["TestHospitalisationID"][$i]));
                }
            }
        }
        ...
        if ($is_next_record || ($EmptyRowsLeft && $this->InsertAllowed)) {
            do {
                ...
                if (!($this->FormSubmitted) && $is_next_record) {
                    ...
                    $this->CachedColumns["TestHospitalisationID"][$this->RowNumber] = $this->DataSource->CachedColumns["TestHospitalisationID"];
                    ...
                    $this->CachedColumns["TestHospitalisationID"][$this->RowNumber] = "";
                ...
                if ($is_next_record) {
                    if ($this->FormSubmitted) {
                        ...
                        if (($this->DataSource->CachedColumns["TestCodeID"] == $this->CachedColumns["TestCodeID"][$this->RowNumber]) && ($this->DataSource->CachedColumns["TestID"] == $this->CachedColumns["TestID"][$this->RowNumber]) && ($this->DataSource->CachedColumns["TestHospitalisationID"] == $this->CachedColumns["TestHospitalisationID"][$this->RowNumber])) {
...
//DataSourceClass_Initialize Event @343-83EBBB06
...
//SetValues Method @343-CFDC2656
    function SetValues()
    {
        $this->CachedColumns["TestCodeID"] = $this->f("TestCodeID");
        $this->CachedColumns["TestID"] = $this->f("TestID");
        $this->CachedColumns["TestHospitalisationID"] = $this->f("TestHospitalisationID");
...
//Insert Method @343-4D3B167F
    function Insert()
    {
        ...
        $this->cp["HospitalisationID"] = new clsSQLParameter("urlHospitalisationID", ccsInteger, "", "", CCGetFromGet("HospitalisationID", NULL), NULL, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["HospitalisationID"]->GetValue()) and !strlen($this->cp["HospitalisationID"]->GetText()) and !is_bool($this->cp["HospitalisationID"]->GetValue())) 
            $this->cp["HospitalisationID"]->SetText(CCGetFromGet("HospitalisationID", NULL));
...
//Update Method @343-5E804E23
    function Update()
    {
        ...
        $wp->AddParameter("1", "dsTestHospitalisationID", ccsInteger, "", "", $this->CachedColumns["TestHospitalisationID"], "", false);
        if(!$wp->AllParamsSet()) {
        ...
        if (!is_null($this->cp["TestResultDetails"]->GetValue()) and !strlen($this->cp["TestResultDetails"]->GetText()) and !is_bool($this->cp["TestResultDetails"]->GetValue())) 
            $this->cp["TestResultDetails"]->SetValue($this->TestResultDetails->GetValue(true));
        $wp->Criterion[1] = $wp->Operation(opEqual, "TestHospitalisationID", $wp->GetDBValue("1"), $this->ToSQL($wp->GetDBValue("1"), ccsInteger),false);
        $Where = 
             $wp->Criterion[1];
...
//Delete Method @343-7C84B691
    function Delete()
    {
        ...
        $this->CmdExecution = true;
        $wp = new clsSQLParameters($this->ErrorBlock);
        $wp->AddParameter("1", "urlTestHospitalisationID", ccsInteger, "", "", CCGetFromGet("TestHospitalisationID", NULL), "", false);
        if(!$wp->AllParamsSet()) {
            $this->Errors->addError($CCSLocales->GetText("CCS_CustomOperationError_MissingParameters"));
        }
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "testhosplist_maint.html";
$BlockToParse = "main";

...
//Connection Settings @0-4055DFF3
...
//Initialize Common Variables @0-FA93957C
...
$CCSLocales->AddLocale("ro", Array("ro", "RO", array(1, 0, ""), 2, ",", ".", array("ianuarie", "februarie", "martie", "aprilie", "mai", "iunie", "iulie", "august", "septembrie", "octombrie", "noiembrie", "decembrie"), array("ian.", "feb.", "mar.", "apr.", "mai.", "iun.", "iul.", "aug.", "sep.", "oct.", "nov.", "dec."), array("duminică", "luni", "marţi", "miercuri", "joi", "vineri", "sâmbătă"), array("D", "L", "Ma", "Mi", "J", "V", "S"), array("D", "L", "M", "M", "J", "V", "S"), array("dd", ".", "mm", ".", "yyyy"), array("d", " ", "mmmm", " ", "yyyy"), array("HH", ":", "nn"), array("HH", ":", "nn", ":", "ss"), "", "", 1, false, "", "utf-8", "UTF-8"));
...
$CCSLocales->AddLocale("uk", Array("uk", "UA", array(1, 0, ""), 2, ",", " ", array("Січень", "Лютий", "Березень", "Квітень", "Травень", "Червень", "Липень", "Серпень", "Вересень", "Жовтень", "Листопад", "Грудень"), array("Січ", "Лют", "Бер", "Кві", "Тра", "Чер", "Лип", "Сер", "Вер", "Жов", "Лис", "Грг"), array("неділя", "понеділок", "вівторок", "середа", "четвер", "п'ятниця", "субота"), array("Нд", "Пн", "Вв", "Ср", "Чт", "Пт", "Сб"), array("Н", "П", "В", "С", "Ч", "П", "С"), array("dd", ".", "mm", ".", "yyyy"), array("d", " ", "mmmm", " ", "yyyy", " р."), array("H", ":", "nn"), array("H", ":", "nn", ":", "ss"), "", "", 1, false, "", "utf-8", "UTF-8"));
$CCSLocales->DefaultLocale = strtolower("uk");
$CCSLocales->Init();
$Charset = "utf-8";
...
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", 'ds7Wfn815yN42JeR');
...
//registry_db Connection Class @-7FD72D9E
class clsDBregistry_db extends DB_Adapter
{
    function clsDBregistry_db()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["registry_db"]);
        parent::Initialize();
...
//End registry_db Connection Class
...
//CCLoginUser @0-00BA13D5
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBregistry_db();
    $SQL = "SELECT LoginID, GroupID, password FROM login WHERE username=" . $db->ToSQL($login, ccsText) . " AND password=MD5(" . $db->ToSQL($password, ccsText) . ")";
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("LoginID"));
...
// can the current facility user see the patient with ID $paId?
function canSeePatient($paId) {
	$can = 0;
	$db = new clsDBregistry_db();
    $SQL = "SELECT patient.PatientID FROM ((((patient LEFT JOIN pregnancy ON pregnancy.PatientID = patient.PatientID) LEFT JOIN visit ON visit.PregnancyID = pregnancy.PregnancyID) LEFT JOIN hospitalisation ON hospitalisation.PregnancyID = pregnancy.PregnancyID) LEFT JOIN referral ON referral.VisitID = visit.VisitID) LEFT JOIN referralhosp ON referralhosp.HospitalisationID = hospitalisation.HospitalisationID WHERE ( pregnancy.FacilityID IN (" . $db->ToSQL(CCGetSession("s_Facilities"), ccsInteger) . ") OR referralhosp.FacilityID IN (" . $db->ToSQL(CCGetSession("s_Facilities"), ccsInteger) . ") OR patient.FacilityID IN (" . $db->ToSQL(CCGetSession("s_Facilities"), ccsInteger) . ") OR referral.FacilityID IN (" . $db->ToSQL(CCGetSession("s_Facilities"), ccsInteger) . ")) AND patient.PatientID = " . $db->ToSQL($paId, ccsInteger); 
	$db->query($SQL);
    if($db->num_rows() > 0) 
		$can = 1; // can see patient if she is found by the query
 	return $can;
}

//require_once(RelativePath ."/Common.inc.php");  //Change to CCS code! DBs are now defined externally

//	 echo CCGetSession('TIMEOUT');
//		echo "pp";
...
{ 
	...
	$timeout = mktime($hour,$mins,$sec+$i_time,$mon,$day,$year);

	// if TIMEOUT session exists & user has not been active
	if((CCGetSession('TIMEOUT') != 0) && (CCGetSession('TIMEOUT') < $current_time))
	{
		// if not he/she will be logged out
		CCSetSession('TimeoutMsg',1);
		header('Location: ' . 'dashboard.php');
		CCLogoutUser();	
	} else 
		CCSetSession('TIMEOUT',$timeout);
} else
	CCSetSession('TIMEOUT', 0); // if we are on login page, timeout is back to 0, so that when logging in again the user is not logged out 

...
// function that allows to name the tooltip of the options in a select HTML element as the displayed text of the option
// this is useful for the select lists in which the text of each option is not fully visible
function addToolTipsSelect(selectName)
{
	//get select
	var select = document.getElementsByName(selectName)[0];
	//get list of options
	var optionsSelect = select.getElementsByTagName('option');
	//set attibute title (for the tooltip) the same as the option text
	for(i=0; i<optionsSelect.length; i++) 
	{
   		optionsSelect[i].title = optionsSelect[i].innerHTML;
	}
